...
[ExecuteInEditMode] public class CommandBufferBlurRefraction : MonoBehaviour {
	public Shader m_BlurShader;
	Material m_Material;
	Camera m_Cam;

	// We'll want to add a command buffer on any camera that renders us so have a dictionary of them.
	private Dictionary<Camera,CommandBuffer> m_Cameras = new Dictionary<Camera,CommandBuffer>();

	// Remove command buffers from all cameras we added into
	private void Cleanup() {
		foreach (var cam in m_Cameras)
			if (cam.Key) cam.Key.RemoveCommandBuffer (CameraEvent.AfterSkybox, cam.Value);
		m_Cameras.Clear();
