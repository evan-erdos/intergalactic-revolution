...
public class UniqueShadow : MonoBehaviour {
    [System.Serializable] public class FocusSetup {
        ...
        public float sceneCaptureDistance = 50; }

    [HideInInspector] public Shader uniqueShadowDepthShader;
    public enum Dimension { x256=256, x512=512, x1024=1024, x2048=2048, x4096=4096, x8192=8192 }
    public Dimension shadowMapSize = Dimension.x2048;
    public float cullingDistance = 15;
    ...
    public float lightNearSize = 4;
    public float lightFarSize = 22;
    public float fallbackFilterWidth = 6;
    public int startFocus;
    public FocusSetup[] shadowFoci;

    int m_downscale = 0, m_activeFocus = -1;
    Light m_lightSource;
    ...
    Matrix4x4 m_shadowMatrix, m_shadowSpaceMatrix;
    Camera m_shadowCamera;

    public void SetDownscale(int downscale) {
        m_downscale = downscale; if (m_shadowTexture) { ReleaseTarget(); AllocateTarget(); } }
    ...
    void Awake() {
        if (!ms_shadowTextureFakePoint) {
            ...
            ms_shadowTextureFakePoint.SetPixel(0, 0, new Color(0,0,0,0));
        ...
        foreach (var r in GetComponentsInChildren<Renderer>()) {
            ...
            var hadMaterials = false;
            var sharedMaterials = r.sharedMaterials;
            for (var (i,n)=(0,sharedMaterials.Length); i<n; ++i) {
                ...
                } (sharedMaterials[i], hadMaterials) = (mi, true);
    ...
    void OnValidate() {
        ...
        if (m_materialInstances!=null)
            for (var (i,n)=(0,m_materialInstances.Count); i<n; ++i)
    ...
    void AllocateTarget() {
        m_shadowTexture = new RenderTexture(
            (int)shadowMapSize >> m_downscale, (int)shadowMapSize >> m_downscale,
            16, RenderTextureFormat.Shadowmap, RenderTextureReadWrite.Linear);
    ...
    void SetStaticShaderUniforms(Material m) {
        ...
        m.SetVector("u_UniqueShadowFilterWidth", fallbackFilterWidth * relativeTexelSize *
            new Vector2(1f/(float)(int)shadowMapSize,1f/(float)(int)shadowMapSize));
        ...
        var uniqueShadowBlockerDistanceScale = blockerDistanceScale*focus.radius*0.5f/10f;
    ...
    bool EnsureLightSource() {
        var (hadValidLight,hadCookie) = (m_lightSource, m_lightSource && m_lightSource.cookie);
    ...
    void SetFocus(int idx) {
        ...
        m_shadowCamera.projectionMatrix = GL.GetGPUProjectionMatrix(
            Matrix4x4.Ortho(-focus.radius, focus.radius, -focus.radius, focus.radius, 0, focus.radius*2), false);
        ...
        var (to,zs,zo,db) = (isD3D9?0.5f/(float)(int)shadowMapSize:0f, isD3D?1f:0.5f, isD3D?0f:0.5f, -focus.depthBias);
        m_shadowSpaceMatrix.SetRow(0, new Vector4(0.5f,0,0,0.5f+to));
        m_shadowSpaceMatrix.SetRow(1, new Vector4(0,0.5f,0,0.5f+to));
        m_shadowSpaceMatrix.SetRow(2, new Vector4(0,0,zs,zo+db));
        m_shadowSpaceMatrix.SetRow(3, new Vector4(0,0,0,1));
    }
    ...
    bool CheckVisibility(Camera o) {
        var focus = shadowFoci[m_activeFocus]; UpdateAutoFocus(focus);
        var targetPos = focus.target.position + focus.target.right * focus.offset.x
            + focus.target.up * focus.offset.y + focus.target.forward * focus.offset.z;
        var bounds = new Bounds(targetPos, Vector3.one * focus.radius * 2);
        return (targetPos-o.transform.position).sqrMagnitude<(cullingDistance*cullingDistance)
            && GeometryUtility.TestPlanesAABB(GeometryUtility.CalculateFrustumPlanes(o),bounds); }

    bool CheckCamera(Camera cam) {
        if (cam==Camera.main) return true;
#if UNITY_EDITOR
        if (UnityEditor.SceneView.currentDrawingSceneView)
            if (UnityEditor.SceneView.currentDrawingSceneView.camera==cam) return true;
#endif
        return false;
    }
    ...
    void OnDrawGizmosSelected() {
        ...
        foreach (var f in shadowFoci) {
            ...
            Gizmos.DrawWireSphere(p, f.radius + (f.autoFocus ? f.autoFocusRadiusBias : 0));
        }
    }
}
