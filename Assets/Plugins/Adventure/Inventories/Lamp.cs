...
namespace Adventure.Inventories {
    public class Lamp : Item, IWieldable {
        ...
        void Ignite() {
            if (!IsLit && gameObject.activeInHierarchy && animator) StartCoroutine(Igniting());
        ...
        public virtual void Wear(StoryArgs e=null) {
            Worn = true;
        ...
        public virtual void Stow(StoryArgs e=null) {
            Worn = false;
            if (gameObject.activeInHierarchy)
                StartCoroutine(LateSetBool("worn",Worn));
        ...
        protected override void Awake() { base.Awake();
            ...
            (lights,renderers) = (GetChildren<Light>(), GetChildren<Renderer>());
            Grip = Find("grip"); Grip.parent = transform;
        }
