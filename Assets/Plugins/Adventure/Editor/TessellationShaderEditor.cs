...
namespace UnityEditor {
    internal class TessellationShaderEditor : ShaderGUI {
        private enum WorkflowMode { Distance, EdgeLength }
        ...
        public void FindProperties (MaterialProperty[] props) {
            ...
            if (maxdist != null) m_WorkflowMode = WorkflowMode.Distance;
            else m_WorkflowMode = WorkflowMode.EdgeLength;
        ...
        public override void OnGUI (MaterialEditor materialEditor, MaterialProperty[] props) {
            FindProperties (props); // can be animated, fetch them every event to ensure values are updated
            m_MaterialEditor = materialEditor;
            var material = materialEditor.target as Material;
            ShaderPropertiesGUI (material);
            // Make sure that needed keywords are set up if we're switching some existing material to a new shader
            if (m_FirstTimeApply) { SetKeywords(material); m_FirstTimeApply = false; }
        }
        ...
        public void ShaderPropertiesGUI (Material material) {
            ...
            } else m_MaterialEditor.ShaderProperty(tessellation, Styles.edgelenText.text);
            m_MaterialEditor.ShaderProperty(tessphong, Styles.tessPhongText.text);
            EditorGUILayout.Space();

            GUILayout.Label (Styles.primaryMapsText, EditorStyles.boldLabel);
            if (hasDispTexture) m_MaterialEditor.TexturePropertySingleLine(Styles.albedoText, albedoMap, albedoColor);
            else m_MaterialEditor.TexturePropertySingleLine(Styles.albedoText2, albedoMap, albedoColor);
            ...
            if (hasDispTexture) GUILayout.Label (Styles.displacementText, EditorStyles.boldLabel);
            else m_MaterialEditor.ShaderProperty(displacementto, Styles.dispTileText.text);
        ...
        static void SetKeywords(Material o) {
            SetKeyword (o, "_NORMALMAP", o.GetTexture ("_BumpMap") || o.GetTexture ("_DetailNormalMap"));
            SetKeyword (o, "_DETAIL_MULX2", o.GetTexture ("_DetailAlbedoMap") || o.GetTexture ("_DetailNormalMap"));
            hasDispTexture = o.GetTexture("_DispTex");
            SetKeyword (o, "_DISPALPHA", !o.GetTexture ("_DispTex"));
        }

        static void MaterialChanged(Material material, WorkflowMode workflowMode) { SetKeywords(material); }
