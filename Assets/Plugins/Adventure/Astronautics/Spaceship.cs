...
namespace Adventure.Astronautics.Spaceships {
    public class Spaceship : Adventure.Object, ISpaceship, ICreatable<ShipProfile> {
        ...
        Map<Type,List<Weapon>> blasters = new Map<Type,List<Weapon>>();
        List<FlightMode> modes = new List<FlightMode> { FlightMode.Assisted, FlightMode.Manual };
        ...
        public void Reset() => (IsEnabled, IsAlive, rigidbody.mass, Health, Energy) = (true,true,Mass,MaxHealth,EnergyCapacity);
        public void Alarm() => audio.Play();

        public void Create(ShipProfile o) =>
            (name, Mass, MaxHealth, EnginePower, rollEffect, pitchEffect, yawEffect,
            brakesEffect, thrustEffect, AeroEffect, dragEffect, energyLoss, energyGain,
            EnergyThrust, EnergyCapacity, TopSpeed, maneuveringEnergy, linearEnergy, Pivots, hitSounds,
            modeClip, changeClip, selectClip, hyperspaceClip, alarmClip, explosion, hyperspace) =
                ...
                o.ManeuveringEnergy, o.linearEnergy, o.Pivots, o.hitSounds, o.modeClip, o.changeClip,
        ...
        public void SelectWeapon() {
            ...
            IEnumerator Selecting() {
                ...
                switch ((nextWeapon=(1+nextWeapon)%blasters.Keys.Count)) {
                    case 0: weapons.Add(blasters[typeof(Weapon)].Cast<Weapon>()); break;
                    default: weapons.Add(blasters[typeof(Weapon)].Cast<Weapon>()); break; }
                nextFire = 0;
        ...
        public void ChangeMode() {
            if (IsEnabled) StartSemaphore(ChangingMode);
        ...
        void Awake() {
            ...
            mechanics = GetChildren<IShipPart>();
            parts = new Stack<IDamageable>(GetChildren<IDamageable>());
            blasters[typeof(Weapon)] = GetChildren<Weapon>();
            foreach (var a in blasters) foreach (var b in a.Value) b.gameObject.SetActive(false);
            foreach (var r in GetChildren<Renderer>())
                if (r.name.Contains("destroyed")) destroyable.Add(r); else degradable.Add(r);
        ...
        void OnKill() {
            ...
            IEnumerator Detonating() {
                destroyable.ForEach(o => o.enabled = true);
        ...
        (float,float) CalculateDrag() {
            ...
            drag += rigidbody.velocity.sqrMagnitude*dragEffect/2;
